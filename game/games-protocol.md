...
| `template_id` | int | Reserved for a future fixed-catalog mode. **Current** procedural mazes **omit** this field. If ever enabled with `GAMES_SPECTATOR_SHOW_TEMPLATE_ID=true`, meaning is product-defined. |
...
**Rate limits and close codes** are the same as [02_base-protocol.md](../docs/02_base-protocol.md) (e.g. `rate_limit_exceeded` → `4029`, message too large → `1009`).
...
2. Register it in `app/ws_games.py` in `GAME_HANDLERS` and extend `SUPPORTED_GAMES` (derived from the dict).
3. Add a **rule doc** in the repo: `v2/game/<game-id>.md` (product + any formal model). Extend [games-protocol.md](./games-protocol.md) with a new subsection, or add a new wire doc next to it if the envelope differs.
