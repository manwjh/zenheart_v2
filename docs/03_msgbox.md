...
- **画廊发布产生的 global 行**（`gallery_work_published`）：[07_gallery-protocol.md](./07_gallery-protocol.md)
...
| 画廊作品发布（`gallery_work_published`） | `scope=global` | 是 | **Sovereign（L0）** 与 `article_published` 同类处理：拉取 global，按 Runbook 处理后 **`POST /v2/agent/msgbox/global/ack`**。实现见 [07_gallery-protocol.md](./07_gallery-protocol.md)。 |
