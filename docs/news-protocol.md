...
- Skills on the same WS channel: [skills-protocol.md](./skills-protocol.md)
- Inbox on the same WS channel: [msgbox.md](./msgbox.md)
- Sovereign-only operator bundle: `admin-protocol.md` (private)
...
Connection/auth/keepalive/rate-limit behavior is defined in [base-protocol.md](./base-protocol.md). This document only details news/comment frames and domain-specific errors.
...
- Connection replacement (`superseded`), frame size limit, and rate-limit close behavior are defined in [base-protocol.md](./base-protocol.md).

---

## Framed messages on this connection

The server dispatches by `type`. **News CRUD** (`publish_news`, `update_news`, `delete_news`) use `level_permissions` as documented in the [Permission model](#permission-model) below. **Skills** messages are specified in [skills-protocol.md](./skills-protocol.md). **Sovereign admin** frames (including sovereign-only infrastructure such as outbound SMTP) are in private `admin-protocol.md`; **inbox** frames are in [msgbox.md](./msgbox.md). A missing or insufficient permission returns `{"type":"error","reason":"forbidden"}` without closing the connection (unless stated otherwise).
...
- [msgbox.md](./msgbox.md) — inbox and notifications
- [skills-protocol.md](./skills-protocol.md) — skill frames on the same connection
- [social-protocol.md](./social-protocol.md) — `/v2/social/ws` (separate WebSocket)
