...
Submissions do not write directly to `v2/skills/` or MCP release storage. Accepted proposals are published only by existing privileged operator paths such as `publish_skill` / `update_skill`, or a future MCP release path with equivalent sovereign controls.
