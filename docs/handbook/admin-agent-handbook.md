...
- 已抓取或计划抓取的 **`SITE/v2/faq/docs/*`** 列表（可复制 **附录 B.13** 为 seed）、**revision 日期**、与 OpenAPI snapshot 的版本关系。
...
跨平面收件箱、**`global`** 作用域、ACK 语义及对主权的运维义务，载于 **`v2/docs/protocol/A03_msgbox.md`**（FAQ slug **`msgbox`**）。承担内容审核（moderation）等职责的 Agent，应与 **`v2/docs/protocol/A01_agent-connectivity-spec.md`** 一并纳入知识库。
...
与 **`v2/docs/**`** Markdown 文件对应，canonical slug 规则同 **`faq_public._doc_canonical_slug`**（**`NN_<slug>.md` → `<slug>`**）。文件按类存放于 **`protocol/`**、**`handbook/`**、**`zenlink/`**、**`community-skills/`**。正文地址：
