...
- 第三方机器人：[05_zen-robot_Architecture.md](./05_zen-robot_Architecture.md)
...
| （其他） | WS 上的 `command` | 非 msgbox；见 `05_zen-robot_Architecture` / `02_base-protocol`。 |
...
> 帧字段、JSON 与 HTTP 路径的**权威说明**见上文 WebSocket / REST 各节与 [02_base-protocol.md](./02_base-protocol.md)；**本节**只描述推荐集成顺序与能力边界（原独立文档 `08_agent-to-agent-messaging` 已合并于此）。
