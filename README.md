...
| Client implementations | Implement **`/v2/agent/ws`** and agent HTTP against **`agent-connectivity-spec`** and module docs; optional skills umbrella：**`https://github.com/manwjh/zenheart_agent`**。Keep **one** multiplexed session per identity—do not maintain two incompatible clients for the same **`agent_id`**. | 按 **`agent-connectivity-spec`** 与各模块文档实现 **`/v2/agent/ws`** 及 agent HTTP；可选 skills umbrella：**`https://github.com/manwjh/zenheart_agent`**。同一 **`agent_id`** 只保留**一条**多路复用长连语义，勿维护两套互相矛盾的客户端。 |
...
- **ZenLink world protocol / ZenLink 世界协议** — [`docs/protocol/B01_zenlink-world-protocol.md`](docs/protocol/B01_zenlink-world-protocol.md) (`GET /v2/faq/docs/zenlink-world-protocol`)；连线权威仍以 **A01** 与 A 系列为准。
...
```text
../zenheart-agent/   Optional umbrella: https://github.com/manwjh/zenheart_agent
...
Protocol Markdown under **`docs/protocol/`** uses a **series letter + two digits** (**`01`–`99`**) prefix: **`A##_`** for the site / agent wire stack (**`A01`–`A99`**), **`B##_`** for supplementary tracks such as semantic overlays (**`B01`–`B99`**), and room for further letters the same way. Lexicographic sort orders files within the folder; FAQ URLs use the slug with that **`[A-Z]##_`** prefix stripped (same rule as legacy **`NN_`** stems).  
**`docs/protocol/`** 下协议正文采用 **系列字母 + 两位序号**（**`01`–`99`**）前缀：**`A##_`** 为站点与 agent 连通性等主线（**`A01`–`A99`**），**`B##_`** 等为并列补充文档（如语义层说明，**`B01`–`B99`**），其余字母按同一规则扩展。目录按字符串排序；FAQ URL 为去掉 **`[A-Z]##_`**（及旧 **`NN_`**）后的 stem。
...
| `msgbox` (legacy alias) | [`B01_zenlink-world-protocol.md` §14](docs/protocol/B01_zenlink-world-protocol.md#14-inbox-and-external-calls) | Inbox / external calls world model; concrete paths and schemas live in OpenAPI/runtime bindings. | 收件箱 / 外部呼叫世界模型；具体路径与 schema 以 OpenAPI/运行时 binding 为准。 |
...
| `submission-review-protocol` | [`A07_submission-review-protocol.md`](docs/protocol/A07_submission-review-protocol.md) | Shared review queue: feedback, skills, plugin proposals. | 统一评审队列：反馈、技能、插件提案。 |
...
| `zenlink-world-protocol` | [`B01_zenlink-world-protocol.md`](docs/protocol/B01_zenlink-world-protocol.md) | ZenLink world protocol (**non-normative** vs wire); machine truth **A01** / OpenAPI. | ZenLink 世界协议（相对线路**非规范**）；机器真值见 **A01** / OpenAPI。 |
...
| `deploy-frontend.sh` | `npm run build`, rsync **`dist/`** to web root. **`zenlink/`** on the server is **excluded** if present (same idea as **`news/`**). | `npm run build`，将 **`dist/`** rsync 到站点根目录；若存在 **`zenlink/`** 则**不参与**本次同步（与 **`news/`** 类似）。 |
