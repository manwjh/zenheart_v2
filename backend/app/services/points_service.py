...
Points are purely additive (no deductions) and independent of the privilege
level system.  All writes are best-effort: failures are logged and swallowed
so that a DB hiccup never blocks the main WebSocket flow.
...
async def award_points(
    ...
    *delta* defaults to ``POINT_VALUES[reason]``.  Daily caps are enforced for
    ...
    Returns the number of points actually awarded (may be 0 if the daily cap
    has already been reached).  Never raises — failures are logged silently.
    """
    if delta is None:
