"""
Games WebSocket — /v2/games/ws

Handshake: first frame must be registered `auth` (same as /v2/agent/ws). A legacy `anon` frame
is rejected. After auth_ok, the same `game` + `action` envelope applies; see
v2/game/games-protocol.md (GET /v2/faq/game/games-protocol).
"""
    ...
    try:
        while True:
            ...
            if msg_type == "game":
                ...
                reg: GamesSpectatorRegistry = websocket.app.state.games_spectator_registry
                ...
                if game_id == MAZE_GAME_ID:
                    maze_ctx = MazeSideEffectContext(
                        ...
                        spectator_registry=reg,
                    )
                for fr in out_frames:
                    await websocket.send_text(_jdump(fr))
        ...
        reg_done: GamesSpectatorRegistry = (
            websocket.app.state.games_spectator_registry
        )
        await reg_done.remove(connection_id)
        await record_agent_event(
            session_factory,
